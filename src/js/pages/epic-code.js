    ...
    .then((data) => {
      ...
      const filteredItems =
        next.getAttribute('data-barba-namespace') === 'home'
          ? data.list.filter((item) => symbolsToFilter.includes(item.symbol))
          : data.list

      // console.log("Data fetched from Walbi API:", filteredItems);
    ...
    .then((cmcData) => {
      ...
      coins.forEach((coin) => {
        ...
        if (cryptoDetail && newItem) {
          const assetFullNameElement = newItem.querySelector(
            '[data-assets-fullname]'
          )
